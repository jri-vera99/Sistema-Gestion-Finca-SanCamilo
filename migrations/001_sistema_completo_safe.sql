-- =================================================================
-- migrations/001_sistema_completo_safe.sql
-- Migración incremental segura para MySQL — Finca San Camilo
-- =================================================================

-- -----------------------------------------------------------------
-- PROCEDIMIENTOS ALMACENADOS DE SEGURIDAD (Se eliminan al final)
-- -----------------------------------------------------------------
DELIMITER $$

DROP PROCEDURE IF EXISTS _add_column_safe $$
CREATE PROCEDURE _add_column_safe(
...
BEGIN
    IF NOT EXISTS (
        ...
        IF isUnique THEN
            SET @ddl = CONCAT('ALTER TABLE ', tableName, ' ADD UNIQUE INDEX ', idxName, ' (', idxCols, ')');
        ELSE
            SET @ddl = CONCAT('ALTER TABLE ', tableName, ' ADD INDEX ', idxName, ' (', idxCols, ')');
        END IF;
        PREPARE stmt FROM @ddl;
        EXECUTE stmt;
        DEALLOCATE PREPARE stmt;
...
-- -----------------------------------------------------------------
CALL _add_column_safe('Usuarios', 'activo', 'TINYINT(1) NOT NULL DEFAULT 1');
CALL _add_column_safe('Usuarios', 'es_superadmin', 'TINYINT(1) NOT NULL DEFAULT 0');
CALL _add_column_safe('Usuarios', 'ultima_sesion', 'DATETIME NULL');
CALL _add_column_safe('Usuarios', 'creado_en', 'DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP');
CALL _add_column_safe('Usuarios', 'actualizado_en', 'DATETIME NULL ON UPDATE CURRENT_TIMESTAMP');
CALL _add_column_safe('Usuarios', 'mfa_habilitado', 'TINYINT(1) NOT NULL DEFAULT 0');
CALL _add_column_safe('Usuarios', 'mfa_secreto', 'VARCHAR(64) NULL COMMENT ''TOTP secret''');

-- Intentar crear índice único para correos (fallará si hay duplicados, en ese caso resolver manualmente y descomentar)
-- CALL _add_index_safe('Usuarios', 'uk_usuarios_correo', 'correo', TRUE);

CALL _add_column_safe('Roles', 'activo', 'TINYINT(1) NOT NULL DEFAULT 1');
CALL _add_column_safe('Roles', 'creado_en', 'DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP');

CALL _add_column_safe('Colmenas', 'observaciones', 'TEXT NULL');
CALL _add_column_safe('Colmenas', 'activa', 'TINYINT(1) NOT NULL DEFAULT 1');
CALL _add_column_safe('Colmenas', 'creado_en', 'DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP');
CALL _add_column_safe('Colmenas', 'creado_por', 'INT NULL');
CALL _add_column_safe('Colmenas', 'actualizado_en', 'DATETIME NULL ON UPDATE CURRENT_TIMESTAMP');

CALL _add_column_safe('Reservas', 'notas', 'TEXT NULL');
CALL _add_column_safe('Reservas', 'creado_en', 'DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP');
CALL _add_column_safe('Reservas', 'actualizado_en', 'DATETIME NULL ON UPDATE CURRENT_TIMESTAMP');

CALL _add_column_safe('Habitaciones', 'descripcion', 'TEXT NULL');
CALL _add_column_safe('Habitaciones', 'imagen_url', 'VARCHAR(500) NULL');
CALL _add_column_safe('Habitaciones', 'activa', 'TINYINT(1) NOT NULL DEFAULT 1');

CALL _add_column_safe('Clientes', 'creado_en', 'DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP');
...
-- Usuario Superadmin seguro (WHERE NOT EXISTS para el correo)
...
-- Configuración inicial modos
INSERT IGNORE INTO Configuracion_Modo (id_modulo, modo)
SELECT id_modulo, 'real' FROM Modulos;

-- Productos con INSERT IGNORE (gracias al UNIQUE uk_nombre_producto)
...
-- Galería con INSERT IGNORE (gracias al UNIQUE uk_url_imagen)
